...
#include "caffe/layers/triplet2_loss_layer.hpp"
#include "time.h"

namespace caffe {

template <typename Dtype>
__global__ void TripletLossCalc_1(const int n, const int step,const int datacount, const Dtype* bottom_data, Dtype* bottom_diff,const Dtype* label,const Dtype margin,const Dtype * lossweight,Dtype* loss,Dtype* losscount, Dtype * validlosscount,Dtype* dist_matrix) {
  CUDA_KERNEL_LOOP(index, n) {
	...
	for(idx=0;idx<step*step;idx++)
	{
		...
		{//find one triplet
			...
			dist_sq_ap = dist_matrix[ida * step + idp];
			dist_sq_an = dist_matrix[ida * step + idn];
...
template <typename Dtype>
__global__ void TripletLossCalc_2(const int n, const int step,const int datacount, const Dtype* bottom_data, Dtype* bottom_diff,const Dtype* label,const Dtype margin,const Dtype * lossweight,Dtype* loss,Dtype* losscount, Dtype * validlosscount,Dtype* dist_matrix) {
  CUDA_KERNEL_LOOP(index, n) {
	int v=index;
	int ida,idp,idn;
	float dist_sq_ap, dist_sq_an;
...
template <typename Dtype>
__global__ void TripletLossCalc_3(const int n, const int step,const int datacount, const Dtype* bottom_data, Dtype* bottom_diff,const Dtype* label,const Dtype margin,const Dtype * lossweight,Dtype* loss,Dtype* losscount, Dtype * validlosscount,Dtype* dist_matrix) {
  CUDA_KERNEL_LOOP(index, n) {
	int v=index;
	int ida,idp,idn;
	float dist_sq_ap, dist_sq_an;
...
template <typename Dtype>
void Triplet2LossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
	...
	Dtype * dst_matrix = dist_matrix_.mutable_gpu_data();
	Get_dist_matrix_gpu(bottom_data, num, datacount, dst_matrix);
	...
	if(this->layer_param_.triplet_loss_param().step()==3)
	{
		...
		TripletLossCalc_3<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(count, step, datacount,bottom_data, bottom_diff,label,margin,lossweight,loss,losscount,validlosscount,dst_matrix);
	...
	else if(this->layer_param_.triplet_loss_param().step()==2)
	{
		...
		TripletLossCalc_2<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(count, step, datacount,bottom_data, bottom_diff,label,margin,lossweight,loss,losscount,validlosscount,dst_matrix);
		CUDA_POST_KERNEL_CHECK;
	}
	else if(this->layer_param_.triplet_loss_param().step()==1)
	{
		...
		TripletLossCalc_1<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(count, step, datacount,bottom_data, bottom_diff,label,margin,lossweight,loss,losscount,validlosscount,dst_matrix);
		CUDA_POST_KERNEL_CHECK;
	}
	else
...
template <typename Dtype>
void Triplet2LossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {

	this->Backward_cpu(top,propagate_down,bottom);
}

INSTANTIATE_LAYER_GPU_FUNCS(Triplet2LossLayer);
}  // namespace caffe
