...
void Triplet2LossLayer<Dtype>::get_hardest_ignore(const Dtype * fea,const Dtype * label,Dtype* ignore_flag,const int feaNum,const int feaDim)
{
	

	Dtype * subdist = dist_matrix_.mutable_cpu_diff();

	Dtype * sumdist = ignore_flag_.mutable_cpu_diff();
	Dtype * arr = temp_.mutable_cpu_data();
	Dtype thr;
	float ignore_hardest_ratio = this->layer_param_.triplet_loss_param().ignore_hardest_ratio();
	int activelen = this->layer_param_.triplet_loss_param().active_ignorehardest_len();


	int start=0,end=0;
	while(start<feaNum)
	{
		for(end=start+1;end<feaNum;end++)
		{
			...
			else
			{
				...
				if(len>=activelen)
				{
					...
					//int endid = end-1;
					int subfeanum = len;
					const Dtype * subdata = fea + startid * feaDim;
					Get_dist_matrix_cpu(subdata, subfeanum, feaDim, subdist);
					caffe_set(subfeanum,Dtype(0.0),sumdist);
					for(int k=0;k<subfeanum;k++)
					{
						for(int n=0;n<subfeanum;n++)
						{
							sumdist[k]+=subdist[k*subfeanum + n];
						}
						arr[k] = sumdist[k];
						
					}
					std::sort(arr,arr+len);
					int id = (int)((float)len * (1-ignore_hardest_ratio));
					thr = arr[id];
					for(int h=0;h<len;h++)
					{
						if(sumdist[h]>thr)
						{
							ignore_flag[start+h]=1;
						}
					}
...
template <typename Dtype>
void Triplet2LossLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void Triplet2LossLayer<Dtype>::Reshape(
  const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  LossLayer<Dtype>::Reshape(bottom, top);
    ...
    samplecount_.Reshape(bottom[0]->num(), 1, 1, 1);
    dist_matrix_.Reshape(bottom[0]->num()*bottom[0]->num(), 1, 1, 1);
    ignore_flag_.Reshape(bottom[0]->num(), 1, 1, 1);
    temp_.Reshape(bottom[0]->num(), 1, 1, 1);
    caffe_set(ignore_flag_.count(),Dtype(0.0),ignore_flag_.mutable_cpu_data());
    caffe_set(ignore_flag_.count(),Dtype(0.0),ignore_flag_.mutable_cpu_diff());
...
template <typename Dtype>
void Triplet2LossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
	...
	Dtype * dst_matrix = dist_matrix_.mutable_cpu_data();
	Get_dist_matrix_cpu(bottom_data, num, datacount, dst_matrix);

	Dtype* ignore_flag = ignore_flag_.mutable_cpu_data();
	
	if(this->layer_param_.triplet_loss_param().ignore_hardest_ratio()>0)
	{
		get_hardest_ignore(bottom_data,label,ignore_flag,num,datacount);
	}
	...
	for(int i=0;i<num;i++)
	{
		...
		for(int j=i+1;j<num;j++)
		{
			...
			{	
				...
				if(this->layer_param_.triplet_loss_param().ignore_hardest_ratio()>0)
				{
					if(ignore_flag[i]>0 || ignore_flag[j]>0||ignore_flag[k]>0)
						continue;
				}
				...
				{//find a triplet
					...
					for(int swap=0;swap < 2;swap++)
					{
						...
						dist_sq_ap = dst_matrix[ida * num + idp];
						dist_sq_an = dst_matrix[ida * num + idn];
...
template <typename Dtype>
void Triplet2LossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(Triplet2LossLayer);
#endif

INSTANTIATE_CLASS(Triplet2LossLayer);
REGISTER_LAYER_CLASS(Triplet2Loss);
