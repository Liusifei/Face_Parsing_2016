...
#include "caffe/layers/gatelstm_layer.hpp"
...
void GateLstmLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    ...
    //w*x
    flps += 2* num_output_ * channels_ *(T_ -1) * col_length_ *3;
    //w*h
    flps += 2* num_output_ * num_output_ *(T_ -1) * col_length_ *3;
    //+b
    flps += num_output_ * col_length_ *(T_ -1) *3;
    //.*g
    flps += num_output_ * col_length_ *(T_ -1) *3;
    //add wx and (wh+b)*g
    flps += num_output_ * col_length_ *(T_ -1) *3;
    ...
    flps += num_output_ * col_length_ *(T_ -1) *3;
    //h=o.*c
    flps += num_output_ * col_length_ *(T_ -1);
...
void GateLstmLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
    ...
    if(this->layer_param_.gatelstm_param().printall_datadiff())
    {
        //print data
